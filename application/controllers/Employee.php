...
class Employee extends CI_Controller {
	...
	{
		$this->db->select('*');
        $this->db->from('job_postion');
	    $query = $this->db->get(); 
        $data['records'] = $query->result(); 
        $this->load->view('backend/jobavail',$data); 
	}
		public function Applied()
	{
		$this->db->select('applied_candidate.candidate_name,job_postion.title,job_postion.description,applied_candidate.email,applied_candidate.phone');
         $this->db->from('applied_candidate');
        $this->db->join('job_postion', 'applied_candidate.jid = job_postion.jid');
	    $query = $this->db->get(); 
        $data['records'] = $query->result(); 
        $this->load->view('backend/applied',$data); 
	}
	...
	$blood = $this->input->post('blood');		
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (!$this->upload->do_upload('image_url')) {
                ...
                redirect("employee/view2?I=" .base64_encode($eid));
			}
    ...
    public function view(){
        ...
        $data['socialmedia'] = $this->employee_model->GetSocialValue($id);
            $year = date('Y');
      //  $data['Leaveinfo'] = $this->employee_model->GetLeaveiNfo($id,$year);
        $this->load->view('backend/employeeprofile',$data);
        }
    else{
		redirect(base_url() , 'refresh');
	}         
